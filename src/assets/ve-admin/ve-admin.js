...
let ClickTab = function clickTab() {
    ...
    //获取对应的iframe节点
    let iframe_box = document.getElementsByClassName("veadmin-content-iframe-box")[0];
...
function showTab(tab_node, iframe_node) {
    //显示标签页
    //遍历标签，全部取消active
    let tab_box = document.getElementsByClassName("veadmin-content-tab")[0];
...
function addTab(title, url) {
    ...
    li_node.style = "background-image: url(" + require('@/assets/ve-admin/images/tabBackground.png') + ")";
    //绑定事件
    ...
    i_node.style = "background-image: url(" + require('@/assets/ve-admin/images/tabBackground-close.png') + ")";
    let em_node = document.createElement("em");
    em_node.style = "background-image: url(" + require('@/assets/ve-admin/images/tabBackground.png') + ")";
    ...
    iframe_node.src = url;
    //div_node.appendChild(iframe_node);
    iframe_box.appendChild(iframe_node);
...
function ShowOrHideCloseAllTabsButton() {
    let tab_box = document.getElementsByClassName("veadmin-content-tab")[0];
    let ul_node = tab_box.firstChild;
    //如果标签大于3个，则显示关闭全部的按钮
    if (ul_node.childNodes.length > 3) {
        //显示【关闭全部】按钮
        document.getElementsByClassName("veadmin-content-button-close-all")[0].style = "display:block";
    } else {
        //显示【关闭全部】按钮
        document.getElementsByClassName("veadmin-content-button-close-all")[0].style = "display:none";
    }
}